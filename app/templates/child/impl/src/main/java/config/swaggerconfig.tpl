package {{data.package}}.impl.config;
...
public class SwaggerConfig {
    ...
    public Docket eDesignApi(SwaggerConfigProperties swaggerConfigProperties) {
        return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo(swaggerConfigProperties)).enable(Boolean.valueOf(swaggerConfigProperties.getEnabled())).select().apis(RequestHandlerSelectors.basePackage("{{data.package}}"))
                .paths(PathSelectors.any()).build().pathMapping("/").groupName("{{data.name}}").directModelSubstitute(LocalDate.class, String.class)
